  ...
  return <Row gutter={[16, 16]}>
    <Col xs={24} xl={12}><Card title="作者卡（来源可验证，不按姓名强制合并）" loading={loading}>
      {authors.length === 0 ? <Empty description="尚无可验证作者卡；不会根据姓名猜测作者身份或贡献。" /> : <List dataSource={authors} renderItem={(author) => <List.Item>
        <Card size="small" style={{ width: '100%' }} title={<Space wrap><Typography.Text strong>{author.canonical_name}</Typography.Text><Tag>{author.identity_status}</Tag>{author.position ? <Tag>作者顺序 #{author.position}</Tag> : null}</Space>}>
          <Descriptions size="small" column={1}>
            <Descriptions.Item label="ORCID">{text(author.orcid)}</Descriptions.Item>
            <Descriptions.Item label="OpenAlex / S2 ID">{text(author.openalex_id ?? author.semantic_scholar_id)}</Descriptions.Item>
    ...
    <Col xs={24} xl={12}><Card title="数据集卡（仅展示当前文本支持的字段）" loading={loading}>
      {datasets.length === 0 ? <Empty description="尚未从当前论文证据中识别出数据集；不会自行补造样本规模、切分或许可证。" /> : <List dataSource={datasets} renderItem={(dataset) => <List.Item>
        <Card size="small" style={{ width: '100%' }} title={<Space wrap><Typography.Text strong>{dataset.canonical_name}</Typography.Text><Tag>{dataset.identity_status}</Tag>{dataset.evidence_level && <Tag color="blue">{dataset.evidence_level}</Tag>}</Space>}>
          <Descriptions size="small" column={1}>
            <Descriptions.Item label="原文提及">{text(dataset.raw_mention)}</Descriptions.Item>
            <Descriptions.Item label="角色 / 任务">{text(dataset.role)} / {text(dataset.task)}</Descriptions.Item>
            ...
            <Descriptions.Item label="Train / Validation / Test">{text(dataset.train_split)} / {text(dataset.validation_split)} / {text(dataset.test_split)}</Descriptions.Item>
