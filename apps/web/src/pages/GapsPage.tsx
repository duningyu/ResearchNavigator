...
export function GapsPage() {
  ...
  return <Space orientation="vertical" size="large" style={{ width: '100%' }}>
    ...
    {scientificDecision && <Card title="证据判断" data-testid="scientific-decision">
      ...
      <Typography.Paragraph>本次比较使用了 {papers.length} 篇你明确选择的论文；未创建候选研究空白，因此不会出现人工确认或研究计划入口。</Typography.Paragraph>
      <Typography.Paragraph strong>你可以：</Typography.Paragraph>
      <List size="small" dataSource={['补充更直接相关的论文', '查看当前证据差异', '保存当前比较结果']} renderItem={(action) => <List.Item>{action}</List.Item>} />
    </Card>}
